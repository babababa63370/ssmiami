...
// Empêche le crash du module WebAssembly lors de l'évaluation du pont Poki-Unity
window.initPokiBridge = function() {
    console.log("[PokiStub] -> window.initPokiBridge() appelé par Unity. Activation du pont fictif.");
    
    // On crée l'objet intermédiaire que Subway Surfers va essayer de manipuler après l'initialisation du pont
    window.pokiBridge = {
        isAdBlocked: false,
        getLanguage: function() { return "fr"; },
        getAdBlockerStatus: function() { return false; },
        commercialBreak: function(callback) {
            console.log("[PokiStub] Demande de pub interceptée via le pont.");
            if (callback) callback();
        },
        rewardedBreak: function(callback) {
            console.log("[PokiStub] Demande de pub récompensée. Attribution du bonus d'office.");
